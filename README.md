...
Production-ready starter template for building modern Rails apps with a Vue 3 frontend, connected via Inertia.js. No API mode, no vue-router — Rails owns routing, Vue owns rendering.
...
1. Replace `MyApp` with your app name in locale files (`app/frontend/i18n/locales/`)
2. Update database credentials in `config/database.yml` and `docker-compose.yml`
3. Update CI config in `.github/workflows/ci.yml` if you change DB credentials
